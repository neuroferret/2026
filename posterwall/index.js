...
const goToRoom = (roomName) => {
  location.href = `https://jitsi.deuxfleurs.fr/NeuroFerret2026_${roomName}`;
}
...
const displayPosters = (posters) => {
  ...
  for (const p of posters) {
    ...
    svg += `<g id="nf26-${id}" transform="translate(${p.x},${p.y - 13 }) scale(0.2)">
      ...
      fill="rgba(255,255,255,0.4)" onclick="openJit('nf26-${id}', '${name}');" style="cursor:pointer; pointer-events: visiblePainted"></path>
      ...
      fill="white" stroke-width="5" onclick="openJit('nf26-${id}', '${name}');" style="cursor:pointer; pointer-events: visiblePainted"></path>
